...
| **Restore original demo state** | Clear all → full deterministic reseed | No |
...
- `tables`: all local entity tables including machines, inventory, stock movements, customizer simulations
...
4. Full `reset()` then `importBackupData()` — no duplicate rows on repeat restore (same IDs replaced).
