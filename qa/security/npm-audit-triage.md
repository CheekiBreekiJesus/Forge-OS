...
> **Integration update (2026-07-04):** Playwright remediated (`@playwright/test@1.61.1`). `xlsx` replaced with `exceljs@4.4.0` + adapter on `integration/dependency-security-remediation`. See `qa/security/dependency-integration-result.md`.
...
|---------|-------------------|-------|
| Playwright SSL (×2 audit nodes) | **Done** (`1.61.1` on `fix/playwright-audit-remediation`) | **Composer** |
| `xlsx` advisories | **Done** (`exceljs@4.4.0` + adapter on `integration/dependency-security-remediation`) | **Codex** (integration review) |
