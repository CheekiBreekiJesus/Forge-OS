import type { Cell, CellHyperlinkValue, CellRichTextValue, CellValue, Workbook } from "exceljs";
...
export type LoadedSpreadsheetWorkbook = {
  ...
  _workbook: Workbook;
  _excelJs: ExcelJSNamespace;
};

let excelJsModulePromise: Promise<ExcelJSNamespace> | null = null;

export function resetSpreadsheetParserEngineForTests(): void {
  excelJsModulePromise = null;
}

export function isSpreadsheetParserEngineLoadedForTests(): boolean {
  return excelJsModulePromise !== null;
}

async function loadExcelJsModule(): Promise<ExcelJSNamespace> {
...
export function extractSheetMatrix(
  ...
  worksheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
    ...
    for (let columnIndex = 1; columnIndex <= boundedColumns; columnIndex += 1) {
      ...
      if (cell.type === workbook._excelJs.ValueType.Formula) {
        formulaCount += 1;
      }
      values.push(display);
    }
...
function formatCellForDisplay(cell: Cell, excelJs: ExcelJSNamespace): string {
  ...
  switch (cell.type) {
    ...
    case excelJs.ValueType.Hyperlink: {
      const hyperlink = cell.value as CellHyperlinkValue;
      return String(hyperlink.text ?? hyperlink.hyperlink ?? "").trim();
    }
    case excelJs.ValueType.RichText: {
      const richText = cell.value as CellRichTextValue;
