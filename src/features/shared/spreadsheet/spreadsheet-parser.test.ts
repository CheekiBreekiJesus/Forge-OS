...
import {
  ...
  resetSpreadsheetParserEngineForTests,
  isSpreadsheetParserEngineLoadedForTests,
  safeObjectFromRow,
...
describe("spreadsheet parser security boundary", () => {
  ...
  it("defers ExcelJS loading until the first workbook parse", async () => {
    resetSpreadsheetParserEngineForTests();
    expect(isSpreadsheetParserEngineLoadedForTests()).toBe(false);
    const buffer = await buildSyntheticXlsxBuffer([
      { name: "Lazy", rows: [["Header"], ["Value"]] }
    ]);
    await loadSpreadsheetWorkbook(buffer);
    expect(isSpreadsheetParserEngineLoadedForTests()).toBe(true);
  });
