-- Inventory runtime RPCs: barcode records, stock mutations, reservations, audit logging.

create table if not exists public.inv_barcode_records (
  ...
  constraint inv_barcode_records_status_check check (status in ('active', 'inactive', 'replaced'))
);

create unique index if not exists inv_barcode_records_tenant_normalized_active_idx
  on public.inv_barcode_records (tenant_id, normalized_value)
  where status = 'active';

create index if not exists inv_barcode_records_tenant_item_idx
  on public.inv_barcode_records (tenant_id, item_id);

alter table public.inv_barcode_records enable row level security;

create policy "tenant members can manage inv barcode records"
on public.inv_barcode_records for all
using (tenant_id in (select public.current_user_tenant_ids()))
with check (tenant_id in (select public.current_user_tenant_ids()));

create or replace function public.inv_write_activity_log(
  p_tenant_id uuid,
...
create or replace function public.inv_physical_quantity(
  p_tenant_id uuid,
  p_item_id uuid,
  p_location_id uuid,
...
as $$
  select coalesce(sum(e.quantity_delta), 0)
    from public.inv_ledger_entries e
   where e.tenant_id = p_tenant_id
     and e.item_id = p_item_id
     and e.location_id = p_location_id
     and e.stock_condition = p_stock_condition;
$$;

create or replace function public.inv_reserved_quantity(
  p_tenant_id uuid,
  p_item_id uuid,
...
as $$
  select coalesce(sum(r.quantity), 0)
    from public.inv_reservations r
...
create or replace function public.inv_available_quantity(
  p_tenant_id uuid,
  p_item_id uuid,
  p_location_id uuid,
  p_stock_condition text default 'available'
...
set search_path = public
as $$
  select public.inv_physical_quantity(p_tenant_id, p_item_id, p_location_id, p_stock_condition)
       - public.inv_reserved_quantity(p_tenant_id, p_item_id, p_location_id);
$$;
...
grant execute on function public.inv_write_activity_log(uuid, text, text, text, text, jsonb) to service_role;
